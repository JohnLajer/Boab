...
class Applyonline_Public {
	...
	public function enqueue_scripts() {
        ...
        wp_enqueue_script( $this->plugin_name.'-formhelper-date', 'https://cdnjs.cloudflare.com/ajax/libs/bootstrap-formhelpers/2.3.0/js/bootstrap-formhelpers.js', array( 'jquery','jquery-ui-datepicker' ), '', false );
            ...
            <form class="aol_app_form" name="aol_app_form" id="aol_app_form" enctype="multipart/form-data">
                <?php
                    ...
                    if(isset($field['required']) AND $field['required'] == '1') :
                        ...
                        $required = $req_class = null;
                    endif;

                    $strPlaceholder     = esc_html(!empty($field['placeholder']) ? $field['placeholder'] : '');
                    ...
                    switch ($field['type']){
                        ...
                        case 'phone':
                            ...
                            // If this is a currency, let's publish it as a number field with attached icon
                            ...
                            if($field['type'] == 'currency')
                            {
                                ...
                                $strAttributes      = 'min="0" step="0.01" data-number-to-fixed="2" data-number-stepfactor="100"';
                            }
                            ...
                            if($field['type'] == 'phone')
                            {
                                ...
                                $strClasses = 'input-medium bfh-phone';

                                if(!empty($field['options'][0])) :
                                    $strAttributes='data-format="'.$field['options'][0].'"';
                                else:
                                    // If nothing is defined, then just act like an ordinary text field
                                    $strAttributes='';
                                    $strClasses = '';
                                endif;
                            ...
                            <div class="form-group row">
                                <div class="col-xs-12 col-sm-3">
                                    <label for="'.$field['key'].'" class="col-2 col-form-label">'.str_replace('_',' ',$field['key']).$required.'</label>
                                </div>
                                <div class="col-xs-12 col-sm-9 input-group">
                                    '.$inputGroupAddon.'
                                    <input class="form-control '.$strClasses.'" type="'.$field['type'].'" name="_aol_app_'.$field['key'].'" value="" '.$strAttributes.' placeholder="'.$strPlaceholder.'" id="'.$field['key'].'" '.$req_class. ' '.$strClasses.'>
                                </div>
                            </div>
                            ';
                            break;
                        ...
                        case 'text_area':
                            ...
                            <div class="form-group row">
                                <div class="col-xs-12 col-sm-3">
                                    <label for="'.$field['key'].'">'.str_replace('_',' ',$field['key']).$required.'</label>
                                </div>
                                <div class="col-xs-12 col-sm-9 input-group">
                                    <textarea name="_aol_app_'.$field['key'].'" class="form-control" id="'.$field['key'].'" '.$req_class.' placeholder="'.$strPlaceholder.'"></textarea></div>
                                </div>
                            ';
                            break;
                        ...
                        case 'dropdown':
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-xs-12 col-sm-9 input-group">
                                    <div id="'.$field['key'].'" >
                                        <select name="_aol_app_'.$field['key'].'" id="'.$field['key'].'" class="form-control" '.$req_class.'>';
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-xs-12 col-sm-9 input-group">
                                    ...
                                    foreach ($options as $option) {
                                        echo '
                                            <label>
                                                <input type="'.$field['type'].'" name="_aol_app_'.$field['key'].'" class="" id="'.$field['key'].'" value="'.$option.'"  '.$this->aol_ad_is_checked($i).'>
                                                '.$option.'
                                            </label>
                                        ';
                                        $i++;
