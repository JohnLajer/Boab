...
class Applyonline_Admin {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->plugin_name, 'https://ajax.googleapis.com/ajax/libs/jqueryui/1.12.1/themes/smoothness/jquery-ui.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name.'-jquery-min', 'https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js', [], '3.2.1', false );
		wp_enqueue_script( $this->plugin_name.'-jquery-ui-min', 'https://ajax.googleapis.com/ajax/libs/jqueryui/1.12.1/jquery-ui.min.js', [], '3.2.1', false );
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/applyonline-admin.js', [], $this->version, false );
	}
 ...
 class Applyonline_MetaBoxes{
	 ...
	 */
        ...
        public function __construct() {
            $this->app_field_types = array('text'=>'Text Field', 'email'=>'E Mail', 'number'=>'Number', 'currency'=>'Currency ($)', 'text_area'=>'Text Area', 'phone'=>'Phone', 'date'=>'Date', 'checkbox'=>'Check Box', 'dropdown'=>'Drop Down', 'radio'=> 'Radio', 'file'=>'File');
	 ...
	 */
            ...
            <tbody>
                <tr>
                    <td class="left" id="newmetaleft">
                        <input type="text" id="adapp_name" placeholder="Fieldname" />
                    </td><td>
                ...
                <tr>
                    <td colspan="3">
                        <input id="adapp_field_placeholder" class="adapp_field_type" style="width:280px;" type="text" placeholder="Custom placeholder" >
                    </td>
                </tr>
                <tr>
                    <td colspan="2" style="display: none;position:relative;" colspan="2">
                        <input id="adapp_field_options" class="adapp_field_type" style="width:280px;" type="text" placeholder="Option1, Option2, Option3" >
                    </td>
                    <td>
                ...
                jQuery('document').ready(function($){
                    ...
                    $('#adapp_field_type').change(function(){
                       ...
                       if(fieldType == 'checkbox' || fieldType == 'dropdown' || fieldType == 'radio'){
                           $('#adapp_field_options').attr('placeholder', 'Option1, Option2, Option3');
                           $('#adapp_field_options').parents('td').siblings('td').html('');
                           $('#adapp_field_options').parents('td').show();

                           $('#adapp_field_placeholder').parents('td').hide();
                           $('#adapp_field_placeholder').val('');
                       }
                       else if( fieldType == 'phone' ) {
                           $('#adapp_field_placeholder').parents('td').show();
                           $('#adapp_field_options').parents('td').show();
                           $('#adapp_field_options').attr('placeholder', 'Default: dd dddd-dddd');
                           $('#adapp_field_options').val('dd dddd-dddd');
                           $('#adapp_field_options').parents('td').siblings('td').html('Example: 01 1234 5678');
                       }
                       else{
                           $('#adapp_field_placeholder').parents('td').show();

                           $('#adapp_field_options').parents('td').hide();
                           $('#adapp_field_options').attr('placeholder', 'Option1, Option2, Option3');
                           $('#adapp_field_options').parents('td').siblings('td').html('');
                    ...
                    $('#addField').click(function(){
                        ...
                        var fieldPlaceholder    = $('#adapp_field_placeholder').val() == '' ? $('#adapp_name').val() : $('#adapp_field_placeholder').val();
                        var style;
                        ...
                        if(fieldName != ''){
                            ...
                            if(fieldType=='text' || fieldType=='email' || fieldType=='date' || fieldType == 'text_area'|| fieldType == 'file' || fieldType == 'number' || fieldType == 'currency'){
                                ...
                                stylePlaceholder = '';
                                styleOptions = '';
                                optionsPlaceholder = 'Default: dd dddd-dddd';
                            }
                            else {
                                styleOptions = "";
                            ...
                            // Before we append this row, let's find out what number it will get

                            $('table#form-fields tbody').append('' +
                                '<tr>' +
                                    '<td>' +
                                        '<input class="orderNo" type="hidden" name="_aol_app_'+fieldName+'[orderNo]" value="0" />' +
                                        '<span class="orderNo">0</span>.' +
                                    '</td>' +
                                    '<td><div class="button-primary toggle-required">Required</div></td>' +
                                    '<td>' +fieldNameRaw + '</td>' +
                                    '<td><select class="adapp_field_type" name="_aol_app_'+fieldName+'[type]">'+fieldTypeHtml+'</select></td>' +
                                    '<td><input type="text" class="options" name="_aol_app_'+fieldName+'[options]" value="'+fieldOptions+'" placeholder="' + optionsPlaceholder + '" style="width:100%;min-width:75px;'+styleOptions+'" /></Td>' +
                                    '<td><input type="text" class="adapp_field_options" name="_aol_app_'+fieldName+'[placeholder]" value="'+fieldPlaceholder+'" class="placeholder" placeholder="Custom Placeholder" style="width:100%;min-width:75px;'+stylePlaceholder+'" /></td>' +
                                    '<td><div class="button removeField">Delete</div></td>' +
                                '</tr>'
                            );

                            // The newly added <tr> must be the last one, let's set the right option value
                            $('table#form-fields tbody').find('tr:last select.adapp_field_type').val(fieldType);
                    ...
                    $('table#form-fields').on('change', 'select.adapp_field_type',function(){

                        var fieldType=$(this).val();

                        if(fieldType == 'checkbox' || fieldType == 'dropdown' || fieldType == 'radio'){
                           $(this).parents('tr').find('input.options').show();
                           $(this).parents('tr').find('input.placeholder').hide();
                            $(this).parents('tr').find('input.options').val();
                            $(this).parents('tr').find('input.placeholder').val();
                        }
                        else if(fieldType == 'phone') {
                            $(this).parents('tr').find('input.options').show();
                            $(this).parents('tr').find('input.placeholder').show();
                            $(this).parents('tr').find('input.options').val('dd dddd-dddd');
                            $(this).parents('tr').find('input.placeholder').val('');
                        }
                        else{
                            $(this).parents('tr').find('input.options').hide();
                            $(this).parents('tr').find('input.placeholder').show();
                            $(this).parents('tr').find('input.options').val('');
                            $(this).parents('tr').find('input.placeholder').val('');
                        }
                    ...
                    $('.adpost_fields').on('click', '.removeField',function(){

                        var parentTable = $(this).parents('table').attr('id');

                        $(this).parents('tr').remove();

                        reassignNumbers(parentTable);
                    });
                    
                    //Toggle Required
                    $('.adpost_fields').on('click', '.toggle-required', function(){
                    ...
                    var fixHelperModified = function(e, tr) {
                        ...
                        updateIndex = function(e, ui) {
                            reassignNumbers($('td', ui.item.parent()).parents('table').attr('id'));
                    };

                    $("table#form-fields tbody").sortable({
                        helper: fixHelperModified,
                        stop: updateIndex
                    }).disableSelection();

                    // Reassign numbers to each row
                    function reassignNumbers(tableID) {

                        var i = 0;
                        $('#'+tableID).find('tr').each(function() {
                            $(this).find('td input.orderNo').val(i);
                            $(this).find('td span.orderNo').html(i);
                            i++;
                        });
                    }
        ...
        public function RenderAdPostFields($post, $bIsFromSettings = false)
        {
            ...
            $fields = $this->sortFieldsArr($fields);
            ...
            foreach($fields as $key => $val):
                ...
                if(in_array($val['type'], array('checkbox','dropdown','radio',))):
                    $strPlaceholder = '<input type="text" class="placeholder" name="'.$key.'[placeholder]" value="" placeholder="Custom Placeholder" style="width:100%;min-width:75px;display:none;" />';
                    $strOptions     = '<input type="text" class="options" name="'.$key.'[options]" value="'.$val['options'].'" placeholder="Option1, option2, option3" style="width:100%;min-width:75px;" />';
                elseif(in_array($val['type'], array('phone'))): // Phone is the only one that shows both fields
                    $placeholderVal = isset($val['placeholder']) ? $val['placeholder'] : '';
                    $strPlaceholder = '<input type="text" class="placeholder" name="'.$key.'[placeholder]" value="'.$placeholderVal.'" placeholder="Custom Placeholder" style="width:100%;min-width:75px;" />';
                    $strOptions     = '<input type="text" class="options" name="'.$key.'[options]" value="'.$val['options'].'" placeholder="Default: dd dddd-dddd" style="width:100%;min-width:75px;" />';
                else:
                    $placeholderVal = isset($val['placeholder']) ? $val['placeholder'] : '';
                    $strPlaceholder = '<input type="text" class="placeholder" name="'.$key.'[placeholder]" value="'.$placeholderVal.'" placeholder="Custom Placeholder" style="width:100%;min-width:75px;" />';
                    $strOptions     = '<input type="text" class="options" name="'.$key.'[options]" placeholder="Option1, option2, option3" style="width:100%;min-width:75px;display:none;"  />';
                endif;

                $iOrderNo = isset($val['orderNo']) ? intval($val['orderNo']) : 0;

                $rows .= '
                    <tr>
                        <td>
                            <input class="orderNo" type="hidden" name="'.$key.'[orderNo]" value="'.$iOrderNo.'" />
                            <span class="orderNo">'.$iOrderNo.'</span>.
                        </td>
                        <td>
                            <input type="hidden" name="'.$key.'[required]" value="'.($val['required']).'" />
                            <div class="button-primary '.$req_class.' toggle-required">Required</div>
                        </td>
                        <td>'.str_replace('_',' ',substr($key,9)).'</td>
                        <td><select class="adapp_field_type" name="'.$key.'[type]">'.$fields.'</select></td>
                        <td>'.$strOptions.'</td>
                        <td>'.$strPlaceholder.'</td>
                        <td><div class="button removeField">Delete</div></td>
                    </tr>
                    ';
            endforeach;
            ...
            return '
            <table border="0" cellspacing="5" cellpadding="0" style="width:100%" id="form-fields">
            <thead>
                <tr>
        ...
        public function sortFieldsArr($fields)
        {
            // Obtain a list of columns
            $orderNo = [];
            $bFailed = false;
            foreach ($fields as $key => $row) {
                if(!isset($row['orderNo'])) {
                    $bFailed = true;

                    break;
                }
                $orderNo[$key]  = $row['orderNo'];
            }

            if(!$bFailed)
                array_multisort($orderNo, SORT_ASC, $fields);
                    ...
                    <div id="templates" class="tab-data">
                        ...
                        <div class="app_form_fields adpost_fields">
                            <?php
                                ...
                                echo $this->RenderAdPostFields($keys, true);
                            ?>
                        </div>  
